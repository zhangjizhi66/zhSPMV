...
SIMDMAT<T, U>::SIMDMAT(CSRMAT<T>& csrmat) {
    ...
    int Nsimd = 0;
    for (int i = 0; i < nrows - nrows_inrow_simd; i += SIMD_WIDTH / 2)
        Nsimd += rows[i].length;
    for (int i = nrows - nrows_inrow_simd; i < nrows; i++)
        Nsimd += std::ceil((double)rows[i].length / SIMD_WIDTH) * 2;

    nsimdblocks_crossrow = std::ceil((double)(nrows - nrows_inrow_simd) / (SIMD_WIDTH / 2));
    nsimdblocks_inrow = nrows_inrow_simd * 2;

    posix_memalign((void**)&nnz_val, VEC_LENGTH / 8, Nsimd * SIMD_WIDTH * sizeof(T));
    posix_memalign((void**)&col_idx, SIMD_WIDTH * 32 / 8, Nsimd * SIMD_WIDTH * sizeof(U));
...
void SPMV_SIMD_complex(SIMDMAT<T, U> &simdmat, T *x, T *y, int nthreads = 1) {
    ...
    for (int isimd = 0; isimd < end_simd; isimd++) {
        ...
        IDX_VEC_TYPE row_idx_vec = IDX_VEC_TYPE::load_unaligned(&simdmat.sort_raw_complex[start_row]);
    ...
    // the last cross-row simd block
    {
        ...
        if ( simdmat.nrows_crossrow_simd % (SIMD_WIDTH / 2) != 0 ) {
            double y_arr[SIMD_WIDTH];
            y_vec.store_unaligned(y_arr);
            int end_i = 2 * (simdmat.nrows_crossrow_simd % (SIMD_WIDTH / 2));
            for (int i = 0; i < end_i; i++)
                y[simdmat.sort_raw_complex[isimd * SIMD_WIDTH + i]] = y_arr[i];
        } else {
    ...
    for (int isimd = simdmat.nsimdblocks_crossrow; isimd < nsimdblocks; isimd++) {
        ...
        y[simdmat.sort_raw_complex[start_row_idx_inrow * 2 + isimd - simdmat.nsimdblocks_crossrow]] = xsimd::reduce_add(y_vec);
#else
        int raw_row_idx = simdmat.sort_raw_complex[start_row_idx_inrow * 2 + isimd - simdmat.nsimdblocks_crossrow];
        y[raw_row_idx] = 0;
...
template <typename T>
void SPMV_CSR_complex(CSRMAT<T> &csrmat, T *x, T *y, int nthreads = 1) {
...
int main(int argc, char *argv[]) {
    ...
    DATATYPE *y1 = (DATATYPE*)malloc(2 * csrmat.nrows * sizeof(DATATYPE));
    DATATYPE *y2 = (DATATYPE*)malloc(2 * csrmat.nrows * sizeof(DATATYPE));
    for (int i = 0; i < csrmat.ncols; i++)
        x[i] = static_cast<DATATYPE>(1);
    for (int i = 0; i < 2 * csrmat.nrows; i++) {
        y1[i] = 0;
        y2[i] = 0;
    }
    ...
    // SIMD warmup
    for (int i = 0; i < 100; i++)
        SPMV_SIMD_complex<DATATYPE, INDEXTYPE, Arch>(simdmat, x, y2, nthreads);
    ...
    SPMV_SIMD_complex<DATATYPE, INDEXTYPE, Arch>(simdmat, x, y2);
    ...
    double time_temp = -1;
    if ( std::filesystem::exists("perf/"+mtxname+"_xsimd"+std::to_string(VEC_LENGTH)+"d_complex_test.txt") ) {
        std::ifstream ifs("perf/"+mtxname+"_xsimd"+std::to_string(VEC_LENGTH)+"d_complex_test.txt");
        double a;
        ifs >> a >> time_temp;
        ifs.close();
    }
    ...
    std::ofstream ofs("perf/"+mtxname+"_xsimd"+std::to_string(VEC_LENGTH)+"d_complex_test.txt");
